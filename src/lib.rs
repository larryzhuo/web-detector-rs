                ...
                if let Some(&max_freq) = hash_freq.values().max() {
                    if max_freq >= 3 {
                        ...
                        let has_img = children.iter().any(|c| {
                            c.descendants().any(|n| {
                                n.value().is_element() && get_node_name(n.value()) == "img"
                            })
                        });
                        let has_link = children.iter().any(|c| {
                            c.descendants().any(|n| {
                                n.value().is_element() && get_node_name(n.value()) == "a"
                            })
                        });

                        let score = (max_freq as i32) * 10
                            + if has_img { 5 } else { 0 }
                            + if has_link { 3 } else { 0 };
